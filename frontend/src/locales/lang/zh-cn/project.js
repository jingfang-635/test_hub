export default {
  project: {
    // List page
    projectManagement: '项目管理',
    newProject: '新建项目',
    searchPlaceholder: '搜索项目名称',
    statTotal: '项目总数',
    statAiGeneration: 'AI用例生成项目',
    statAiIntelligent: 'AI智能测试项目',
    statApi: 'API项目',
    statUi: 'UI项目',
    statApp: 'APP项目',
    statusFilter: '状态筛选',
    projectName: '项目名称',
    description: '描述',
    status: '状态',
    owner: '负责人',
    createdAt: '创建时间',
    actions: '操作',

    // Status
    active: '进行中',
    paused: '已暂停',
    completed: '已完成',
    archived: '已归档',

    // Dialog
    editProject: '编辑项目',
    createProject: '新建项目',
    projectNamePlaceholder: '请输入项目名称',
    projectDescription: '项目描述',
    projectDescriptionPlaceholder: '请输入项目描述',
    selectStatus: '请选择状态',
    projectTypes: '项目类型',
    projectTypesHint: '可多选，创建后将自动在各模块中创建项目',
    projectTypesColumn: '模块',
    basicInfo: '基本信息',
    moduleConfig: '模块配置',
    typeAiGeneration: 'AI用例生成',
    typeAiIntelligent: 'AI智能测试',
    typeApiTesting: 'API测试',
    typeUiAutomation: 'UI自动化',
    typeAppAutomation: 'APP自动化',
    typeSelected: '已选择',
    typeSelect: '选择',
    noProjectTypes: '未关联',
    moduleContent: '模块内容',
    enterModule: '进入模块',
    typeAiGenerationDesc: '智能分析需求文档，自动生成测试用例',
    typeAiIntelligentDesc: '基于自然语言的智能化测试执行',
    typeApiTestingDesc: '接口自动化测试与环境、用例管理',
    typeUiAutomationDesc: 'Web UI 自动化脚本与执行管理',
    typeAppAutomationDesc: '移动端 APP 自动化测试与设备管理',
    update: '更新',
    create: '创建',

    // Validation
    projectNameRequired: '请输入项目名称',
    projectNameLength: '项目名称长度在 2 到 200 个字符',
    projectStatusRequired: '请选择项目状态',
    projectTypesRequired: '请至少选择一个项目类型',

    // Messages
    fetchListFailed: '获取项目列表失败',
    updateSuccess: '项目更新成功',
    createSuccess: '项目创建成功',
    updateFailed: '项目更新失败',
    createFailed: '项目创建失败',
    deleteConfirm: '确定要删除这个项目吗？',
    deleteConfirmWithVersions: '确定删除项目「{name}」及其关联版本吗？此操作不可恢复。',
    deleteSuccess: '项目删除成功',
    deleteFailed: '项目删除失败',

    // Detail page
    projectDetail: '项目详情',
    projectInfo: '项目信息',
    noDescription: '暂无描述',
    projectMembers: '项目成员',
    addMember: '添加成员',
    username: '用户名',
    email: '邮箱',
    role: '角色',
    joinedAt: '加入时间',
    removeMember: '删除',
    environments: '环境配置',
    addEnvironment: '添加环境',
    environmentName: '环境名称',
    baseUrl: '基础URL',
    loginUsername: '登录账号',
    loginPassword: '登录密码',
    loginUsernamePlaceholder: '测试环境登录账号（可选）',
    loginPasswordPlaceholder: '测试环境登录密码（可选）',
    authStateSaved: '登录态已保存',
    authStateUpdatedAt: '登录态更新时间',
    defaultEnvironment: '默认环境',
    yes: '是',
    no: '否',
    environmentNameRequired: '请输入环境名称',
    baseUrlRequired: '请输入基础URL',
    environmentAddSuccess: '环境添加成功',
    environmentAddFailed: '环境添加失败',
    editEnvironment: '编辑环境',
    environmentUpdateSuccess: '环境更新成功',
    environmentSaveFailed: '环境保存失败',
    environmentDeleteConfirm: '确定要删除该环境吗？',
    environmentDeleteSuccess: '环境删除成功',
    environmentDeleteFailed: '环境删除失败',
    fetchDetailFailed: '获取项目详情失败',
    memberDeleteSuccess: '成员删除成功',
    memberDeleteFailed: '删除成员失败'
  },
  home: {
    // Header
    user: '用户',
    logout: '退出登录',
    logoutConfirm: '确定要退出登录吗？',
    logoutSuccess: '已退出登录',

    // Language
    language: {
      current: '中文',
      zhCN: '简体中文',
      en: 'English'
    },

    // Title
    title: 'TestHub 测试平台',
    subtitle: '一站式智能化测试解决方案',

    // Cards
    aiCaseGeneration: '用例管理',
    aiCaseGenerationDesc: '智能分析需求，自动生成测试用例',
    apiTesting: '接口测试',
    apiTestingDesc: '高效的接口自动化测试与管理',
    uiAutomation: 'UI自动化测试',
    uiAutomationDesc: '可视化的Web UI自动化测试',
    dataFactory: '数据工厂',
    dataFactoryDesc: '灵活的测试数据构造与管理',
    aiIntelligentMode: 'AI 智能模式',
    aiIntelligentModeDesc: '基于自然语言的智能化测试执行',
    aiEvaluator: 'AI评测师',
    aiEvaluatorDesc: '基于评测师知识库，提供专业软件测试问答',
    configCenter: '配置中心',
    configCenterDesc: '系统环境、AI模型及通知配置',

    // Messages
    featureInDevelopment: '功能正在开发中......'
  },
  profile: {
    // Page
    title: '个人设置',
    basicInfo: '基本信息',
    changePassword: '修改密码',

    // Basic Info
    username: '用户名',
    email: '邮箱',
    name: '姓名',
    department: '部门',
    position: '职位',

    // Password
    currentPassword: '当前密码',
    newPassword: '新密码',
    confirmPassword: '确认密码',
    changePasswordButton: '修改密码'
  }
}
