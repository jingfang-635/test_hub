...
class BaseBrowserAgent:
    ...
    async def _verify_execution_llm(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"Execution LLM unavailable: AI 执行模型连接失败"
                f"（model={self.model_name}, base_url={self.base_url}）。详情: {e}"
            ) from e

    async def _preflight_check(self):
        """
        执行前预检：配置、浏览器、模型连通性。
        任一失败立刻终止并抛出明确错误，避免先开浏览器再连续空转失败。
        """
        import platform

        errors = []
        if not self.api_key:
            errors.append("未配置 AI 模型 API Key（角色 browser_use_text）")
        if not self.model_name:
            errors.append("未配置 AI 模型名称（角色 browser_use_text）")
        ...
        if errors:
            raise RuntimeError(f"执行预检失败: {'；'.join(errors)}")

        try:
            await self._verify_execution_llm()
        except Exception as e:
            detail = str(e)
            if 'Execution LLM unavailable' in detail:
                raise RuntimeError(f"执行预检失败: {detail}") from e
            raise RuntimeError(f"执行预检失败: AI 执行模型连接失败。详情: {detail}") from e

        logger.info(
            f"✅ 执行预检通过 (model={self.model_name}, chrome={chrome_path or 'auto'})"
        )
    ...
    def _local_break_down_task(self, task_description: str):
        """
        本地启发式拆分：模型连接失败或只返回单步时作为兜底，
        尽量拆出多条业务步骤，避免整段任务被压成一条。
        """
        text = str(task_description or '').strip()
        if not text:
            return []

        explicit_steps = self._extract_structured_steps(text)
        if len(explicit_steps) >= 2:
            return self._finalize_steps(explicit_steps, text)

        # 统一分隔符，保留 URL / 引号内容不被误伤
        working = text.replace('\r\n', '\n').replace('\r', '\n')
        working = re.sub(r'[；;]+', '\n', working)
        working = re.sub(r'(?:然后|之后|接着|再然后|再)\s*', '\n', working)

        action_verbs = (
            r'访问|打开|进入|点击|双击|输入|填写|选择|勾选|搜索|查询|登录|注册|'
            r'提交|保存|关闭|切换|验证|确认|检查|查看|等待|滚动|上传|下载|'
            r'删除|新增|创建|编辑|修改|返回|跳转|刷新|断言|截图'
        )
        # 在动作词前断句（避免把首段动作切开）
        working = re.sub(rf'(?<!^)(?<![\n])(?=(?:{action_verbs}))', '\n', working)
        ...
        for raw_line in working.split('\n'):
            ...
            if not line:
                continue
            # 过短碎片合并到上一条
            if candidates and len(line) < 4 and not re.search(action_verbs, line):
                candidates[-1] = f"{candidates[-1]}{line}"
            else:
        ...
        finalized = self._finalize_steps(candidates, text)
        if len(finalized) >= 2:
            return finalized

        # 最后保底：至少返回规范化后的显式步骤或原文
        normalized = self._normalize_steps(explicit_steps or candidates, text)
        return normalized or [text]

    def _prefer_richer_steps(self, primary_steps, fallback_steps):
        """当模型结果过粗（尤其是单步）而本地兜底更细时，优先本地结果。"""
        primary = primary_steps or []
        fallback = fallback_steps or []
        if len(fallback) >= 2 and len(primary) <= 1:
            return fallback
        if len(fallback) > len(primary) + 1 and len(primary) <= 2:
            return fallback
    ...
    async def analyze_task(self, task_description: str):
        local_steps = self._local_break_down_task(task_description)
        try:
            ...
            if len(explicit_steps) >= 2:
                if self._should_redecompose_explicit_steps(explicit_steps):
                    try:
                        ...
                        cleaned_steps = self._prefer_richer_steps(cleaned_steps, local_steps)
                    except Exception as model_error:
                        logger.warning(f"⚠️ model recompose failed, using local fallback: {model_error}")
                        cleaned_steps = local_steps if len(local_steps) >= 2 else self._normalize_steps(
                            explicit_steps, task_description
                        )
                else:
            ...
            try:
                ...
                logger.warning(f"⚠️ model break_down failed, using local fallback: {model_error}")
                cleaned_steps = local_steps

            if not cleaned_steps:
                cleaned_steps = local_steps or [task_description.strip()]

            return [{'id': i + 1, 'description': s, 'status': 'pending'} for i, s in enumerate(cleaned_steps)]
        except Exception as e:
            logger.warning(f"⚠️ analyze_task fallback triggered: {e}")
            cleaned_steps = local_steps if local_steps else self._finalize_steps([], task_description)
            return [{'id': i + 1, 'description': s, 'status': 'pending'} for i, s in enumerate(cleaned_steps)]
    ...
    async def run_task(self, task_description: str, planned_tasks=None, callback=None, should_stop=None, skip_preflight=False):
        ...
        final_task += "12. CREDENTIALS RULE: NEVER invent, replace, or guess credentials. Only use the username/password explicitly provided in the task. If login keeps failing with an explicit error like '登录失败' or '用户名或密码错误', stop retrying after at most 3 attempts and mark the current login task as failed.\n"
        ...
        agent = Agent(
            ...
            max_retries=1,  # 单步动作重试次数
            max_failures=3,  # 连续失败最多重试 3 次后终止，避免无限空转
