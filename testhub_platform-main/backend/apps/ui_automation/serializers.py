...
class UiScheduledTaskSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if trigger_type is None and self.instance:
            trigger_type = self.instance.trigger_type

        if trigger_type in UiScheduledTask.CRON_BASED_TRIGGER_TYPES:
            cron_expression = attrs.get('cron_expression')
            if cron_expression is None and self.instance:
                cron_expression = self.instance.cron_expression
            ...
            try:
                ...
                if not croniter.is_valid(cron_expression):
                    raise serializers.ValidationError("Cron表达式格式无效")
            ...
            interval_seconds = attrs.get('interval_seconds')
            if interval_seconds is None and self.instance:
                interval_seconds = self.instance.interval_seconds
            ...
            execute_at = attrs.get('execute_at')
            if execute_at is None and self.instance:
                execute_at = self.instance.execute_at
            if not execute_at:
        ...
        if task_type is None and self.instance:
            task_type = self.instance.task_type
        if task_type == 'TEST_SUITE':
            test_suite = attrs.get('test_suite', getattr(self.instance, 'test_suite', None) if self.instance else None)
            if not test_suite:
                raise serializers.ValidationError("测试套件不能为空")
        elif task_type == 'TEST_CASE':
            test_cases = attrs.get('test_cases')
            if test_cases is None and self.instance:
