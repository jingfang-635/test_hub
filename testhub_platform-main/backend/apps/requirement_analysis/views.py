...
from .models import (
    ...
    GenerationConfig, AIModelService, KnowledgeBase, KnowledgeDocument, KnowledgeBaseLLMConfig
)
from .serializers import (
    ...
    TestCaseGenerationRequestSerializer, LegacyTestCaseGenerationRequestSerializer,
    TestCaseReviewRequestSerializer, FeishuFetchRequestSerializer, FigmaFetchRequestSerializer,
    AIModelConfigSerializer, PromptConfigSerializer, TestCaseGenerationTaskSerializer,
    GenerationConfigSerializer, KnowledgeBaseSerializer,
    KnowledgeDocumentSerializer, KnowledgeDocumentUploadSerializer,
    KnowledgeBaseLLMConfigSerializer
)
from .feishu_client import FeishuClient, FeishuError
from .figma_client import FigmaClient, FigmaError
from .services import RequirementAnalysisService, DocumentProcessor

logger = logging.getLogger(__name__)


def execute_generation_task(task_id):
    """
    执行测试用例生成任务（模块级函数，可被线程或 SSE 请求内同步调用）。
    - 在非 Vercel 环境：由 daemon thread 后台执行
    - 在 Vercel 环境：由 stream_progress SSE 请求同步触发（利用 SSE 长连接保持函数生命周期）
    """
    ...
    try:
        ...
        if not task:
            logger.error(f"execute_generation_task: 任务不存在 task_id={task_id}")
            return

        # 原子乐观锁：只有 new/pending/generating(超时僵尸) 状态才能抢占，避免重复执行
        # 用原生 SQL 做原子 UPDATE，保证多线程/多实例下只有一个能抢到执行权
        from django.db import connection
        locked = False
        try:
            with connection.cursor() as cursor:
                # 允许抢占的条件：new(初始) / pending / 或 generating 但 5 分钟没更新（僵尸任务）
                sql = """
                    UPDATE testcase_generation_task
                    SET status = %s, progress = %s, updated_at = %s
                    WHERE task_id = %s AND (
                        status IN ('new', 'pending')
                        OR (status = 'generating' AND TIMESTAMPDIFF(SECOND, updated_at, NOW()) > 300)
                    )
                    LIMIT 1
                """
                now = timezone.now()
                cursor.execute(sql, ['generating', 10, now, task_id])
                locked = cursor.rowcount > 0
        except Exception as _lock_err:
            logger.warning(f"原子锁SQL失败，降级为非原子检查 task_id={task_id}: {_lock_err}")
            task.refresh_from_db()
            if task.status not in ['new', 'pending']:
                return
            locked = True

        if not locked:
            logger.warning(
                f"execute_generation_task: 任务被其他实例/线程执行或已结束 task_id={task_id}, status={task.status}")
            return
        ...
        try:
            ...
            if task.output_mode == 'stream':
                ...
                # 写库节流状态（按时间/增量，避免每个 chunk 打 DB）
                _stream_save = {'last_t': 0.0, 'last_len': 0}
                STREAM_SAVE_INTERVAL = 1.0  # 秒
                STREAM_SAVE_CHARS = 1500
                ...
                async def stream_callback(chunk):
                    """流式回调：节流写库，降低生成阶段 DB 压力"""
                    task.stream_buffer += chunk
                    task.stream_position = len(task.stream_buffer)
                    now = time.monotonic()
                    grew = task.stream_position - _stream_save['last_len']
                    if now - _stream_save['last_t'] >= STREAM_SAVE_INTERVAL or grew >= STREAM_SAVE_CHARS:
                        try:
                            await async_save_stream_buffer(task.stream_buffer)
                            _stream_save['last_t'] = now
                            _stream_save['last_len'] = task.stream_position
                ...
                task.progress = 60
                task.save(update_fields=['generated_test_cases', 'progress', 'stream_buffer',
                                         'stream_position', 'last_stream_update'])
                ...
                if enable_auto_review and task.reviewer_model_config and task.reviewer_prompt_config:
                    try:
                        ...
                        # 评审内容缓存
                        review_buffer = []
                        _review_save = {'last_t': 0.0, 'last_len': 0}
                        ...
                        async def review_stream_callback(chunk):
                            ...
                            now = time.monotonic()
                            grew = current_length - _review_save['last_len']
                            if now - _review_save['last_t'] >= STREAM_SAVE_INTERVAL or grew >= STREAM_SAVE_CHARS:
                                try:
                                    content = ''.join(review_buffer)
                                    await async_save_review(content)
                                    _review_save['last_t'] = now
                                    _review_save['last_len'] = current_length
                                except Exception as save_error:
                        ...
                        try:
                            ...
                            task.save(update_fields=['status', 'progress', 'final_test_cases'])

                            try:
                                _final_save = {'last_t': 0.0, 'last_len': 0}
                                ...
                                async def final_callback(chunk):
                                    ...
                                    now = time.monotonic()
                                    grew = current_length - _final_save['last_len']
                                    if now - _final_save['last_t'] >= STREAM_SAVE_INTERVAL or grew >= STREAM_SAVE_CHARS:
                                        try:
                                            await async_save_final(task.final_test_cases)
                                            _final_save['last_t'] = now
                                            _final_save['last_len'] = current_length
                                        except Exception as save_error:
                        ...
                        task.save(update_fields=['final_test_cases', 'review_feedback'])
                else:
                    sorted_cases = AIModelService.sort_test_cases_by_id(generated_cases)
                ...
                if enable_auto_review and task.reviewer_model_config and task.reviewer_prompt_config:
                    try:
                        ...
                        try:
                            ...
                            try:
                                _final_save_full = {'last_t': 0.0, 'last_len': 0}
                                STREAM_SAVE_INTERVAL = 1.0
                                STREAM_SAVE_CHARS = 1500
                                ...
                                async def final_callback_full(chunk):
                                    ...
                                    current_length = len(task.final_test_cases)
                                    now = time.monotonic()
                                    grew = current_length - _final_save_full['last_len']
                                    if now - _final_save_full['last_t'] >= STREAM_SAVE_INTERVAL or grew >= STREAM_SAVE_CHARS:
                                        try:
                                            await async_save_final_full(task.final_test_cases)
                                            _final_save_full['last_t'] = now
                                            _final_save_full['last_len'] = current_length
                                        except Exception as save_error:
                                            logger.warning(f"保存最终用例失败: {save_error}")
            ...
            # 完成任务：勿 refresh，避免冲掉内存中已整理的 final_test_cases
        ...
        logger.error(f"生成任务执行失败 task_id={task_id}: {e}")
        try:
            task = TestCaseGenerationTask.objects.filter(task_id=task_id).first()
            if task:
                task.status = 'failed'
...
class BusinessRequirementViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def generate_test_cases(self, request):
        """为选中的需求生成测试用例"""
        serializer = LegacyTestCaseGenerationRequestSerializer(data=request.data)
...
class KnowledgeBaseViewSet(viewsets.ModelViewSet):
    ...
    def documents(self, request, pk=None):
        ...
        documents = knowledge_base.documents.all().order_by('-created_at')
...
class KnowledgeDocumentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = super().get_queryset()
        knowledge_base = self.request.query_params.get('knowledge_base')
        if knowledge_base:
    ...
    def perform_create(self, serializer):
        document = serializer.save()
        self._process_document(document, raise_error=True)

    def _process_document(self, document, raise_error=False):
        """上传后提取文本，并在启用时自动向量化"""
        from .services import KnowledgeVectorizer
        ...
        try:
            ...
            document.save(update_fields=['status', 'is_vectorized', 'chunk_count', 'updated_at'])

            text = DocumentProcessor.extract_text(document)
            document.extracted_text = text or ''
            document.error_message = ''
            document.save(update_fields=['extracted_text', 'error_message', 'updated_at'])

            if document.knowledge_base.enable_vectorization:
                KnowledgeVectorizer.vectorize_document(document)
            ...
            document.is_vectorized = False
            document.save(update_fields=['status', 'error_message', 'is_vectorized', 'updated_at'])
            if raise_error:
    ...
    def vectorize(self, request, pk=None):
        ...
        try:
            ...
            chunk_count = KnowledgeVectorizer.vectorize_document(document)
            document.status = 'ready'
            document.error_message = ''
            document.save(update_fields=['status', 'error_message', 'updated_at'])
            return Response({
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if document.file:
            try:
                ...
                logger.warning(f"删除知识库文档文件失败: {e}")
...
class TestCaseGenerationTaskViewSet(viewsets.ModelViewSet):
    ...
    def generate(self, request):
        ...
        try:
            ...
            task_data = {
                ...
                'source_type': validated_data.get('source_type') or 'manual',
            }
            ...
            source_url = (validated_data.get('source_url') or '').strip()
            if source_url:
            ...
            if task_serializer.is_valid():
                ...
                # 判断环境：Vercel 下 daemon thread 会在请求返回后被杀，所以不启动线程
                # 在 Vercel 下，任务会由用户前端发起 SSE 请求（stream_progress）时同步触发执行
                IS_VERCEL = bool(getattr(settings, 'IS_VERCEL', False) or os.environ.get('VERCEL', ''))

                if not IS_VERCEL:
                    # 本地/自建环境：使用 daemon thread 后台执行
                    import threading

                    def _background_run():
                        execute_generation_task(task.task_id)
                    ...
                    # Vercel Serverless 环境：不启动后台线程
                    # 用户访问 stream_progress SSE 时会同步触发执行（利用 SSE 长连接保活函数生命周期）
                    logger.info(
                        f"Vercel 环境：任务 {task.task_id} 已保存，等待 SSE 请求触发执行（stream_progress）")
    ...
    def stream_progress_sse(self, request, task_id=None):
        ...
        try:
            ...
            # ---- Vercel 兼容：在 SSE 请求内同步触发任务执行 ----
            # 原因：Vercel Serverless 在请求返回后会立即回收实例，daemon thread 会被杀
            # 方案：用户访问 SSE 时，本请求生命周期内同步执行 execute_generation_task（后台线程）
            #       SSE 循环监控数据库进度推送；后台线程执行结束后 SSE 推送 done 退出
            IS_VERCEL = bool(getattr(settings, 'IS_VERCEL', False) or os.environ.get('VERCEL', ''))
            _worker_thread = None
            _worker_started = False

            def _maybe_start_worker():
                """Vercel 环境下在后台线程启动执行，并确保 SSE 请求期间进程不被回收。
                注意：这里仍使用 thread，但由于 SSE 连接保持，**本请求不返回**，所以实例不会被回收。
                """
                nonlocal _worker_thread, _worker_started
                if _worker_started:
                    return
                ...
                try:
                    ...
                    pass
                # 终态或已经在生成中（有后台进程还在跑），不用再启动
                if task.status in ['completed', 'failed', 'cancelled']:
                    return
                # 再检查是否有更新时间很新（30s内），可能是别的客户端启动了但函数没返回
                if task.status == 'generating' and task.updated_at:
                    try:
                        delta = (timezone.now() - task.updated_at).total_seconds()
                        if delta < 60:
                            return  # 60s 内更新过，视为正在执行
                    except Exception:
                        pass
                try:
                    import threading
                    ...
                    def _run():
                        try:
                            ...
                            logger.error(f"SSE worker 执行异常: task_id={task.task_id}: {_e}")

                    _worker_thread = threading.Thread(target=_run, daemon=False)
                    _worker_thread.start()
                    logger.info(
                        f"SSE worker 已启动: task_id={task.task_id}, is_vercel={IS_VERCEL}, status={task.status}")
                except Exception as e:
                    logger.error(f"启动 SSE worker 失败 task_id={task.task_id}: {e}")

            # 无论是否 Vercel：都尝试在 SSE 请求内启动 worker
            # 好处：兼容没开 Celery 的单机场景（默认单机用 daemon thread 可能也会丢任务，这里兜底启动）
            _maybe_start_worker()
            ...
            def event_stream():
                ...
                while True:
                    ...
                    if task.status in ['completed', 'failed', 'cancelled']:
                        ...
                        # 发送最终状态（失败时附带 error_message，便于前端展示）
                        status_payload = {
                            'type': 'status',
                            'status': task.status,
                            'progress': task.progress,
                        ...
                        if task.status == 'failed' and task.error_message:
                            status_payload['error_message'] = task.error_message
                        final_status = json.dumps(status_payload, ensure_ascii=False)
    ...
    def save_to_records(self, request, task_id=None):
        ...
        try:
            ...
            if getattr(task, 'is_saved_to_records', False):
                return Response(
            ...
            return Response({
                'message': '测试用例已保存到AI生成用例记录，请到「AI生成用例」中采纳后才会进入用例库',
                'task_id': task.task_id,
            ...
            try:
                ...
                for case_data in test_cases_data:
                    TestCase.objects.create(
                        ...
                        priority=self._map_priority(case_data.get('priority', 'P2')),
    ...
    def _map_priority(self, priority_str):
        ...
        return priority_map.get(key, 'P2')
...
class KnowledgeBaseLLMConfigViewSet(viewsets.ModelViewSet):
    """知识库大模型配置（向量化 / 内容整理 / 文档解析）"""
    queryset = KnowledgeBaseLLMConfig.objects.all()
    serializer_class = KnowledgeBaseLLMConfigSerializer
    permission_classes = []

    def list(self, request, *args, **kwargs):
        """获取当前配置（单例）"""
        config = KnowledgeBaseLLMConfig.get_active_config()
        if not config:
            return Response({'message': '未找到配置'}, status=status.HTTP_404_NOT_FOUND)
        serializer = self.get_serializer(config)
        return Response(serializer.data)

    def create(self, request, *args, **kwargs):
        """创建配置；若已存在则转为更新（保持单例）"""
        existing = KnowledgeBaseLLMConfig.get_active_config()
        if existing:
...
class FeishuViewSet(viewsets.ViewSet):
    """飞书文档：用户 OAuth + 文档拉取。"""

    @staticmethod
    def _frontend_redirect(status_key: str, message: str = '') -> str:
        from urllib.parse import urlencode, urlparse, urlunparse, parse_qsl

        base = (getattr(settings, 'FEISHU_FRONTEND_REDIRECT', '') or '').strip()
        if not base:
            base = 'http://localhost:3000/ai-generation/requirement-analysis'
        parsed = urlparse(base)
        query = dict(parse_qsl(parsed.query, keep_blank_values=True))
        query['feishu_oauth'] = status_key
        if message:
            query['feishu_msg'] = message[:200]
        return urlunparse(parsed._replace(query=urlencode(query)))

    @action(detail=False, methods=['get'], url_path='oauth_status')
    def oauth_status(self, request):
        """当前登录用户是否已连接飞书。"""
        if not request.user or not request.user.is_authenticated:
            return Response({'connected': False, 'error': '未登录'}, status=status.HTTP_401_UNAUTHORIZED)
        client = FeishuClient(user=request.user)
        data = client.get_oauth_status(request.user)
        return Response(data, status=status.HTTP_200_OK)

    @action(detail=False, methods=['get'], url_path='oauth_authorize_url')
    def oauth_authorize_url(self, request):
        """生成飞书用户授权链接。"""
        if not request.user or not request.user.is_authenticated:
            return Response({'error': '请先登录', 'code': 'login_required'}, status=status.HTTP_401_UNAUTHORIZED)
        try:
            client = FeishuClient(user=request.user)
            result = client.build_authorize_url(request.user.id)
            return Response(result, status=status.HTTP_200_OK)
        except FeishuError as e:
            return Response({'error': e.message, 'code': e.code}, status=e.status_code)
        except Exception as e:
            logger.exception('生成飞书授权链接失败')
            return Response(
                {'error': f'生成授权链接失败: {e}', 'code': 'internal_error'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

    def _complete_oauth_with_code(self, *, code: str, state: str, expected_user=None):
        """校验 state、换 token 并落库。供前端中转与后端直连回调复用。"""
        from apps.users.models import User

        client = FeishuClient()
        state_data = client.pop_oauth_state(state)
        ...
        try:
            ...
            raise FeishuError('用户不存在', code='oauth_failed', status_code=400) from exc

        if expected_user is not None and getattr(expected_user, 'id', None) != user.id:
            raise FeishuError(
                '飞书授权用户与当前登录用户不一致，请重新连接',
                code='oauth_user_mismatch',
                status_code=403,
            )

        token_payload = client.exchange_code_for_token(code)
        client.save_user_tokens(user, token_payload)
        return client.get_oauth_status(user)

    @action(detail=False, methods=['post'], url_path='oauth_complete')
    def oauth_complete(self, request):
        """前端中转完成 OAuth：浏览器带着 code/state 回到前端后，由前端带登录态提交换票。"""
        if not request.user or not request.user.is_authenticated:
            return Response(
                {'error': '请先登录', 'code': 'login_required'},
        ...
        if not code or not state:
            return Response(
                {'error': '缺少 code 或 state', 'code': 'oauth_failed'},
                status=status.HTTP_400_BAD_REQUEST,
            )

        try:
            data = self._complete_oauth_with_code(
                code=code,
                state=state,
            ...
            logger.warning('飞书 OAuth 完成失败: %s', e.message)
            return Response({'error': e.message, 'code': e.code}, status=e.status_code)
        except Exception as e:
            logger.exception('飞书 OAuth 完成异常')
            return Response(
                {'error': f'飞书授权失败: {e}', 'code': 'internal_error'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def oauth_callback(self, request):
        """飞书 OAuth 回调（兼容直连后端）。推荐 redirect 到前端，由 oauth_complete 换票。"""
        from django.http import HttpResponseRedirect

        error = request.query_params.get('error')
        if error:
            return HttpResponseRedirect(self._frontend_redirect('denied', error))

        code = request.query_params.get('code')
        state = request.query_params.get('state')
        if not code or not state:
            return HttpResponseRedirect(self._frontend_redirect('error', '缺少 code 或 state'))

        # 若 redirect_uri 配成了前端页，飞书会把 code 打到前端；此处仅处理直连后端的情况
        try:
            self._complete_oauth_with_code(code=code, state=state)
            return HttpResponseRedirect(self._frontend_redirect('success'))
        except FeishuError as e:
            logger.warning('飞书 OAuth 回调失败: %s', e.message)
            return HttpResponseRedirect(self._frontend_redirect('error', e.message))
        except Exception as e:
            logger.exception('飞书 OAuth 回调异常')
            return HttpResponseRedirect(self._frontend_redirect('error', str(e)[:200]))

    @action(detail=False, methods=['post'], url_path='oauth_disconnect')
    def oauth_disconnect(self, request):
        """解除当前用户的飞书授权绑定。"""
        if not request.user or not request.user.is_authenticated:
            return Response({'error': '请先登录'}, status=status.HTTP_401_UNAUTHORIZED)
        FeishuClient(user=request.user).disconnect_user(request.user)
        return Response({'ok': True, 'message': '已断开飞书连接'}, status=status.HTTP_200_OK)

    @action(detail=False, methods=['post'])
    def fetch(self, request):
        """根据飞书文档/Wiki 链接拉取标题与纯文本正文（使用当前用户 OAuth）。"""
        if not request.user or not request.user.is_authenticated:
            return Response(
                {'error': '请先登录', 'code': 'login_required'},
                status=status.HTTP_401_UNAUTHORIZED,
        ...
        serializer = FeishuFetchRequestSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        try:
            client = FeishuClient(user=request.user)
            result = client.fetch_requirement(serializer.validated_data['url'], user=request.user)
            return Response(result, status=status.HTTP_200_OK)
        except FeishuError as e:
            return Response(
            ...
            logger.exception('飞书文档拉取失败')
            return Response(
                {'error': f'飞书文档拉取失败: {e}', 'code': 'internal_error'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

    @action(detail=False, methods=['post'])
    def test_connection(self, request):
        """校验飞书应用凭证是否可用（tenant，运维自检）。"""
        try:
            client = FeishuClient()
            ...
            return Response(
                {'error': e.message, 'code': e.code, 'ok': False},
                status=e.status_code,
            )
        except Exception as e:
            logger.exception('飞书连接测试失败')
            return Response(
                {'error': f'飞书连接测试失败: {e}', 'code': 'internal_error', 'ok': False},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )


class FigmaViewSet(viewsets.ViewSet):
    """Figma 设计稿：Personal Access Token 拉取文本并用于用例生成。"""

    @action(detail=False, methods=['post'])
    def fetch(self, request):
        """根据 Figma 链接提取标题与纯文本需求正文。"""
        if not request.user or not request.user.is_authenticated:
            return Response(
                {'error': '请先登录', 'code': 'login_required'},
                status=status.HTTP_401_UNAUTHORIZED,
        ...
        serializer = FigmaFetchRequestSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        try:
            result = FigmaClient().fetch_requirement(serializer.validated_data['url'])
            return Response(result, status=status.HTTP_200_OK)
            ...
            logger.exception('Figma 设计稿拉取失败')
            return Response(
                {'error': f'Figma 设计稿拉取失败: {e}', 'code': 'internal_error'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

    @action(detail=False, methods=['post'])
    def test_connection(self, request):
        """校验 FIGMA_ACCESS_TOKEN 是否已配置（不做网络探测）。"""
        client = FigmaClient()
        if not client.access_token:
            return Response(
                {
                    'ok': False,
                    'error': 'Figma 未配置，请设置 FIGMA_ACCESS_TOKEN',
                    'code': 'not_configured',
                },
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )
        return Response({'ok': True, 'message': 'FIGMA_ACCESS_TOKEN 已配置'}, status=status.HTTP_200_OK)