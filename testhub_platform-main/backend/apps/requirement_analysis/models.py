...
class KnowledgeBaseLLMConfig(models.Model):
    ...
    embedding_base_url = models.URLField(
        max_length=500, verbose_name='Embedding Base URL',
        default=DASHSCOPE_BASE_URL
    )
    embedding_model_name = models.CharField(
        max_length=100, verbose_name='Embedding 模型名称',
        default='text-embedding-v3'
    )

    # 内容整理 Refiner（暂仅支持阿里百炼）
    refiner_api_key = models.CharField(max_length=500, verbose_name='Refiner API Key', blank=True, default='')
    refiner_base_url = models.URLField(
        max_length=500, verbose_name='Refiner Base URL',
        default=DASHSCOPE_BASE_URL
    )
    refiner_model_name = models.CharField(
        max_length=100, verbose_name='Refiner 模型名称',
        default='qwen-plus'
    )
    ...
    created_by = models.ForeignKey(
        ...
        related_name='knowledge_llm_configs', verbose_name='创建者'
    )
    created_at = models.DateTimeField(default=timezone.now, verbose_name='创建时间')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
...
class KnowledgeBase(models.Model):
    ...
    is_active = models.BooleanField(default=True, verbose_name='是否启用')
    chunk_size = models.PositiveIntegerField(default=500, verbose_name='分块大小')
    chunk_overlap = models.PositiveIntegerField(default=50, verbose_name='分块重叠')
    enable_vectorization = models.BooleanField(default=True, verbose_name='是否向量化')
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='knowledge_bases',
        verbose_name='创建者', null=True, blank=True
    )
    project = models.ForeignKey(
        Project, on_delete=models.SET_NULL, related_name='knowledge_bases',
        verbose_name='关联项目', null=True, blank=True
    )
    created_at = models.DateTimeField(default=timezone.now, verbose_name='创建时间')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
...
class KnowledgeDocument(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='uploaded_knowledge_documents',
        verbose_name='上传者', null=True, blank=True
    )
    file_size = models.PositiveIntegerField(verbose_name='文件大小(bytes)', null=True, blank=True)
    extracted_text = models.TextField(verbose_name='提取的文本内容', blank=True)
...
class TestCaseGenerationTask(models.Model):
    ...
    source_url = models.URLField(max_length=1000, blank=True, default='', verbose_name='需求来源链接')
...
class FeishuUserAuth(models.Model):
    """TestHub 用户绑定的飞书 OAuth 凭证（user_access_token）。"""
    ...
    token_expires_at = models.DateTimeField(null=True, blank=True, verbose_name='access_token 过期时间')
    refresh_expires_at = models.DateTimeField(null=True, blank=True, verbose_name='refresh_token 过期时间')
    scope = models.TextField(blank=True, default='', verbose_name='授权 scope')
    open_id = models.CharField(max_length=100, blank=True, default='', verbose_name='飞书 open_id')
    union_id = models.CharField(max_length=100, blank=True, default='', verbose_name='飞书 union_id')
    feishu_name = models.CharField(max_length=100, blank=True, default='', verbose_name='飞书显示名')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    ...
    def __str__(self):
        return f"{self.user_id} - {self.feishu_name or self.open_id or 'feishu'}"


class FeishuOAuthState(models.Model):
    """飞书 OAuth state（防 CSRF，落库避免多进程/重启丢失）。"""
    state = models.CharField(max_length=128, unique=True, db_index=True, verbose_name='state')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='feishu_oauth_states')
    ...
    def __str__(self):
        return f'{self.state[:12]}... -> user={self.user_id}'
...
class AIModelService:
    ...
    @staticmethod
    @sync_to_async
    def _load_writer_context(task: 'TestCaseGenerationTask') -> Tuple[str, 'AIModelConfig', str]:
        """在同步线程中加载 writer 相关 ORM 字段，避免 async 上下文中懒加载 FK。"""
        return (
            task.writer_prompt_config.content,
    ...
    @staticmethod
    @sync_to_async
    def _load_reviewer_context(task: 'TestCaseGenerationTask') -> Tuple[str, 'AIModelConfig']:
        """在同步线程中加载 reviewer 相关 ORM 字段，避免 async 上下文中懒加载 FK。"""
        return (
            task.reviewer_prompt_config.content,
    ...
    async def review_test_cases(task: TestCaseGenerationTask, test_cases: str) -> str:
        """评审测试用例"""
        try:
            reviewer_prompt, reviewer_config = await AIModelService._load_reviewer_context(task)
    ...
    async def revise_test_cases_based_on_review(
        ...
        Returns:
            str: 改进后的测试用例
        """
        writer_prompt, writer_config, _requirement_text = await AIModelService._load_writer_context(task)
