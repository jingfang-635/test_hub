...
class AppDeviceViewSet(viewsets.ModelViewSet):
    ...
    def screenshot(self, request, pk=None):
        ...
        功能：
        1. 使用 adb shell screencap 保存截图到设备
        2. 使用 adb pull 拉取到本地（避免 Windows \n→\r\n 二进制损坏）
        3. 转换为 Base64 并返回 data URL 格式
        
        注意：不使用 exec-out 方式，因为 Windows 下二进制管道会导致数据损坏
        """
        device = self.get_object()
        ...
        device_screenshot_path = '/sdcard/__testhub_screenshot.png'
        temp_file = None
        
        try:
            adb_path = get_adb_path()
            
            # 步骤1: 在设备上截图保存为 PNG 文件
            subprocess.run(
                [adb_path, '-s', device.device_id, 'shell', 'screencap', '-p', device_screenshot_path],
                check=True,
                timeout=15,
                **subprocess_kwargs
            )
            ...
            subprocess.run(
                [adb_path, '-s', device.device_id, 'pull', device_screenshot_path, temp_file],
                check=True,
                timeout=15,
                **subprocess_kwargs
            )
            
            # 步骤3: 读取本地文件（完整二进制数据，无损坏）
            ...
            # 验证 PNG 文件头
            if not image_data[:4] == b'\x89PNG':
                logger.warning(f"截图 PNG 头部异常: {image_data[:8].hex()}")
            
            # 步骤4: 转换为 Base64
            image_base64 = base64.b64encode(image_data).decode('utf-8')
            
            logger.info(f"设备 {device.device_id} 截图成功，大小: {len(image_data)} bytes")
            ...
            try:
                subprocess.run(
                    [adb_path, '-s', device.device_id, 'shell', 'rm', device_screenshot_path],
                    timeout=5,
                    **subprocess_kwargs
                )
            except Exception:
