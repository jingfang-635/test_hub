...
from .models import (
    ...
    GenerationConfig, KnowledgeBase, KnowledgeDocument, KnowledgeDocumentChunk,
    KnowledgeBaseLLMConfig, TestCaseGenerationTask, FeishuUserAuth
)
...
@admin.register(KnowledgeBaseLLMConfig)
...
class KnowledgeBaseAdmin(admin.ModelAdmin):
    list_display = ['name', 'is_active', 'project', 'created_by', 'created_at']
...
class KnowledgeDocumentAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'is_vectorized', 'chunk_count', 'uploaded_by', 'created_at'
    ]
    list_filter = ['document_type', 'status', 'is_vectorized', 'created_at']
    search_fields = ['title', 'knowledge_base__name']
    readonly_fields = [
        'file_size', 'extracted_text', 'chunk_count', 'is_vectorized',
...
class KnowledgeDocumentChunkAdmin(admin.ModelAdmin):
    list_display = ['id', 'document', 'knowledge_base', 'chunk_index', 'char_count', 'created_at']
...
@admin.register(FeishuUserAuth)
class FeishuUserAuthAdmin(admin.ModelAdmin):
    list_display = ['user', 'feishu_name', 'open_id', 'token_expires_at', 'updated_at']
    search_fields = ['user__username', 'feishu_name', 'open_id']
    readonly_fields = ['created_at', 'updated_at']