...
class AppTestExecutor:
    ...
    def run_tests(
        ...
        try:
            ...
            # 确保子进程使用新版 ADB（旧版 1.0.32 不支持 HELLO 命令）
            new_adb_path = self._find_newest_adb()
            if new_adb_path:
                adb_dir = os.path.dirname(new_adb_path)
                env['ADB_PATH'] = new_adb_path
                # 将新版 ADB 目录放到 PATH 最前面
                existing_path = env.get('PATH', '')
                env['PATH'] = adb_dir + os.pathsep + existing_path
                logger.info(f"子进程使用新版 ADB: {new_adb_path}")
    ...
    def _find_newest_adb(self) -> Optional[str]:
        """
        查找系统上最新的 ADB 可执行文件路径。
        旧版 ADB (1.0.32) 不支持 airtest 需要的 HELLO 握手命令。
        """
        candidate_paths = [
