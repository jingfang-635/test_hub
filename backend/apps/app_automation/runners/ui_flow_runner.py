...
def _to_int_tuple(value, length=2):
    """
    将任意坐标格式（字符串、列表、元组，且元素可能是 str/float）统一转为 int 元组。
    无法转换时返回 None。
    例如:
        "903, 2210"     -> (903, 2210)
        ["903", "2210"] -> (903, 2210)
        (903.0, 2210.0) -> (903, 2210)
        None / 格式错误  -> None
    """
    if value is None:
        return None
    if isinstance(value, str):
        ...
        if len(parts) < length:
            return None
        try:
            return tuple(int(float(p)) for p in parts[:length])
    ...
    if isinstance(value, (list, tuple)):
        if len(value) < length:
            return None
        try:
            return tuple(int(float(v)) for v in value[:length])
        except (ValueError, TypeError):
            return None
    return None
...
class UiFlowRunner:
    ...
    def _resolve_selector(self, step: Dict[str, Any]) -> Any:
        ...
        if selector_type == 'image':
            ...
            # 坐标选择器
            pos = _to_int_tuple(selector, length=2)
            if pos:
                return pos
            ...
            # 区域选择器（用于 exists 等）
            region = _to_int_tuple(selector, length=4)
            if region:
                return region
    ...
    def _resolve_element_by_id(self, element_id: int) -> Any:
        ...
        try:
            ...
            if element.element_type == 'image':
                ...
                pos = _to_int_tuple((x, y), length=2)
                if not pos:
                    logger.warning(f"元素 {element_id} 的坐标无效: x={x}, y={y}")
                    return None
                return pos
                ...
                region = _to_int_tuple((x1, y1, x2, y2), length=4)
                if not region:
                    logger.warning(f"元素 {element_id} 的区域无效: x1={x1}, y1={y1}, x2={x2}, y2={y2}")
                    return None
                return region
    ...
    def _region_to_point(self, target):
        """将 4 元素区域 (x1,y1,x2,y2) 转换为中心点 (cx,cy)，其他类型原样返回"""
        if isinstance(target, (tuple, list)) and len(target) == 4:
            cx = (int(target[0]) + int(target[2])) // 2
            cy = (int(target[1]) + int(target[3])) // 2
            logger.info(f"区域 {target} 转换为中心点 ({cx}, {cy})")
            return (cx, cy)
        return target
    ...
    def _action_touch(self, step: Dict[str, Any]):
        ...
        target = self._region_to_point(target)
    ...
    def _action_swipe(self, step: Dict[str, Any]):
        """滑动动作"""
        start_raw = step.get('start')
        end_raw = step.get('end')
        duration = step.get('duration', 0.5)
        
        start = _to_int_tuple(start_raw, length=2)
        end = _to_int_tuple(end_raw, length=2)
        
        if not start or not end:
            step_name = step.get('name', 'swipe')
            raise ValueError(f"步骤 '{step_name}' 无法解析滑动坐标: start={start_raw}, end={end_raw}")
    ...
    def _action_drag(self, step: Dict[str, Any]):
        ...
        if start and end:
            start = self._region_to_point(start)
            end = self._region_to_point(end)
    ...
    def _action_swipe_to(self, step: Dict[str, Any]):
        ...
        for i in range(max_swipes):
            ...
            reso = G.DEVICE.get_current_resolution()
            W = int(reso[0])
            H = int(reso[1])
            
            if direction == 'up':
                swipe((W // 2, int(H * 0.7)), (W // 2, int(H * 0.3)))
            elif direction == 'down':
                swipe((W // 2, int(H * 0.3)), (W // 2, int(H * 0.7)))
            elif direction == 'left':
                swipe((int(W * 0.7), H // 2), (int(W * 0.3), H // 2))
            elif direction == 'right':
                swipe((int(W * 0.3), H // 2), (int(W * 0.7), H // 2))
    ...
    def _action_image_exists_click(self, step: Dict[str, Any]):
        ...
        if main_target and exists(main_target):
            ...
            touch(self._region_to_point(main_target))
        elif fallback_target:
            logger.info(f"主定位不存在，点击备用定位")
            touch(self._region_to_point(fallback_target))
    ...
    def _action_foreach_assert(self, step: Dict[str, Any]):
        ...
        try:
            ...
            for i in range(max_loops):
                ...
                # 点击
                touch(self._region_to_point(click_target))
                time.sleep(interval)
