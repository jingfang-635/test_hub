...
def _find_newest_adb():
    """
    查找系统上最新的 ADB 可执行文件路径。
    旧版 ADB (1.0.32) 不支持 HELLO 握手命令，会导致 airtest 连接失败。
    """
    # 常见的新版 ADB 路径（按优先级排序）
    candidate_paths = [
        r"D:\Programs\scrcpy\scrcpy-win64-v4.1\adb.exe",
        r"D:\Programs\leidian\LDPlayer14\adb.exe",
    ]
    
    # 也可以从环境变量中读取
    env_adb = os.environ.get('ADB_PATH') or os.environ.get('ANDROID_HOME')
    if env_adb:
        if os.path.isdir(env_adb):
            candidate_paths.append(os.path.join(env_adb, 'platform-tools', 'adb.exe'))
            candidate_paths.append(os.path.join(env_adb, 'adb.exe'))
        elif os.path.isfile(env_adb):
            candidate_paths.append(env_adb)
    ...
    for path in candidate_paths:
        if os.path.isfile(path):
            try:
                ...
                if '1.0.41' in output or '1.0.40' in output or '37.0' in output or '35.0' in output:
                    logger.info(f"找到新版 ADB: {path}")
                    return path
            except Exception:
                pass
...
# 在导入 airtest 之前设置 ADB 路径
_NEW_ADB_PATH = _find_newest_adb()
if _NEW_ADB_PATH:
    os.environ['ADB_PATH'] = _NEW_ADB_PATH
    # 同时将新版 ADB 所在目录添加到 PATH 最前面，确保 airtest 使用新版
    adb_dir = os.path.dirname(_NEW_ADB_PATH)
    os.environ['PATH'] = adb_dir + os.pathsep + os.environ.get('PATH', '')
    logger.info(f"已设置 ADB 路径: {_NEW_ADB_PATH}")
...
class AirtestBase:
    ...
    def setup_airtest(self, config: Optional[dict] = None) -> bool:
        ...
        try:
            import airtest
            logger.info(f"airtest 版本: {airtest.__version__ if hasattr(airtest, '__version__') else 'unknown'}")
        except ImportError:
            logger.error("airtest 未安装，请运行: pip install airtest>=1.3")
            return False
        ...
        for attempt in range(retry_count):
            try:
                ...
                # 检查是否是 HELLO 命令错误（ADB 协议问题）
                if 'HELLO' in error_str or 'unknown command' in error_str:
                    logger.warning(f"检测到 ADB 协议错误，尝试重启 ADB 服务器...")
    ...
    def _restart_adb_server(self):
        ...
        try:
            # 使用新版 ADB 路径
            adb_cmd = _NEW_ADB_PATH or 'adb'
            logger.info(f"正在重启 ADB 服务器 (使用: {adb_cmd})...")
            # 杀掉 ADB 服务器
            subprocess.run([adb_cmd, 'kill-server'], 
                         capture_output=True, timeout=10, 
                         creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
            time.sleep(1)
            # 启动 ADB 服务器
            result = subprocess.run([adb_cmd, 'start-server'], 
                                  capture_output=True, timeout=10,
                                  creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
            time.sleep(1)
            # 检查设备
            ...
            logger.info(f"ADB 设备列表: {output.strip()}")
        except Exception as e:
            logger.error(f"重启 ADB 服务器失败: {e}")
