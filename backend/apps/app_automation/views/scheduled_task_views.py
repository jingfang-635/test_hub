    ...
    def run_now(self, request, pk=None):
        ...
        try:
            ...
            if task.task_type == 'TEST_SUITE':
                ...
                celery_task = async_task(
                    'apps.app_automation.tasks.execute_app_suite_task',
                ...
                return Response({
                    ...
                    'data': {'task_id': celery_task, 'test_case_count': len(executions)}
                })
                ...
                from django_q.tasks import async_task
                celery_task = async_task(
                    'apps.app_automation.tasks.execute_app_test_task',
                    execution.id,
                    package_name=package_name,
                    scheduled_task_id=task.id,
                )
                execution.task_id = celery_task
                execution.save(update_fields=['task_id'])

                return Response({
                    'success': True,
                    'message': '测试用例开始执行',
                    'data': {'task_id': celery_task}
                })
